...
# Import routes from a 'routes partial' file, into a routes.draw block
...
def import_routes( partial:, plugin: :ShinyCMS )
  ...
  raise ArgumentError, "Partial file '#{partial}' not found" unless File.file? full_path

  content = File.read( full_path )

  instance_eval( content, full_path.to_s )
end
